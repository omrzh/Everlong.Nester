...
**docs/.** `docs/design/` holds the domain specs (a HARD RULE governs what goes in them); `docs/guide/` holds the consumer guides.
...
5. `docs/design/*.md` states the domain's design intent and what the code cannot express — never a restatement of the code, and never a reference to a type, word or document of a domain the code does not depend on.
